...
func AddVideoComment(ctx context.Context, comment *db.Comment) error {
	...
	if err := tx.ZAdd(ctx, i64ToStr(comment.VideoID)+constants.CommentSuffix, redis.Z{
		Score:  float64(comment.ID),
		Member: MalComment,
	}).Err(); err != nil {
		return err
	}
	if err := tx.Expire(ctx, i64ToStr(comment.VideoID)+constants.CommentSuffix, constants.CommentExpTime).Err(); err != nil {
		return err
	}
...
func DelVideoComment(ctx context.Context, comment *db.Comment) error {
	...
	return rComment.ZRemRangeByScore(ctx, i64ToStr(comment.VideoID)+constants.CommentSuffix, score, score).Err()
...
func GetVideoCommentCount(ctx context.Context, videoId int64) (bool, int64, error) {
	v, err := rComment.ZScore(ctx, constants.VideoCommentCountZset, i64ToStr(videoId)).Result()
...
func IncrVideoCommentCount(ctx context.Context, videoId int64) error {
	tx := rComment.TxPipeline()
	if err := tx.ZIncrBy(ctx, constants.VideoCommentCountZset, 1, i64ToStr(videoId)).Err(); err != nil {
		return err
	}
	if err := tx.Expire(ctx, constants.VideoCommentCountZset, constants.VideoExpTime).Err(); err != nil {
		return err
	}
	if _, err := tx.Exec(ctx); err != nil {
		return err
...
func SetVideoCommentCount(ctx context.Context, videoId int64, count int64) error {
	tx := rComment.TxPipeline()
	if err := tx.ZAdd(ctx, constants.VideoCommentCountZset, redis.Z{
		Score:  float64(count),
